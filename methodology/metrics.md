...
### Critical Implementation & Bootstrap Protocol
1. **Calibrated Input Vectors:** The model output probabilities from `Agent.system_one_batch` already reflect temperature scaling and softmax. Applying a redundant second softmax crushes true probabilities (e.g. $0.90 \to 0.54$) and invalidates calibration metrics. Our implementation operates directly on calibrated probability vectors.
2. **Query-Level Clustered Bootstrap:** ECE confidence intervals must NOT be computed as the sample variance of individual query absolute errors $|c_i - a_i|$ (which measures binary scoring variance, not binned calibration). Instead, ECE confidence intervals are generated by query-level clustered bootstrap ($B=1,000$ resamples with replacement of query pairs $(\hat{c}_i, \mathbf{1}[\hat{y}_i = y_i^*])$), re-evaluating the full 15-bin ECE formula on each resampled population to produce the empirical percentile interval.

---

## 8. Multi-Objective Pareto Analysis & Instability Metric Specification

### Objectives
The Pareto frontier evaluates candidate selection architectures across three simultaneous operational dimensions:
1. **Accuracy ($\uparrow$):** Fraction of queries correctly classified ($y^* = \hat{y}$).
2. **Latency ($\downarrow$):** Median query execution time ($p50$ in milliseconds).
3. **Instability ($\downarrow$):** Expected decision volatility under re-evaluation.

### Instability Metric Specification
The Pareto instability objective strictly employs **within-method permutation/residual flip rate ($\mathrm{FR}_{\mathrm{top1}}$)**:
- For stochastic random sampling (`B0_Random`), $\mathrm{FR}_{\mathrm{top1}}$ measures pairwise argmax discordance across random candidate permutations.
- For ensemble marginalization (`Rand_Marg_M*`, `B2_Cyclic_M*`), $\mathrm{FR}_{\mathrm{top1}}$ measures residual decision volatility between two independent $M$-pass ensembles ($\mathrm{ResFlip}_M$).
- For deterministic serialization baselines (`B0_Native`, `B1_Alpha`, `B1_ReverseAlpha`), within-method $\mathrm{FR}_{\mathrm{top1}} \equiv 0.0$ by definition: repeating a deterministic string sorting operation yields identical prompt tokens and zero run-to-run prediction variance.

### Separation from Cross-Canonical Counterfactual Bias
While `B1_Alpha` achieves $0.0\%$ within-method instability, it locks in severe positional exposure bias: reversing the sorting order (`B1_ReverseAlpha`, $Z \to A$) induces a **$55.83\%$ cross-canonical flip rate** at $K=77$. To preserve mathematical precision:
- `FR_top1` strictly quantifies within-method repeatability.
- `CrossCanonicalFlip` is reported as a separate, orthogonal diagnostic of exposure bias, preventing cross-method disagreement from masquerading as run-to-run instability.
